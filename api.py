...
async def lifespan(_: FastAPI) -> AsyncIterator[None]:
    """Start and stop shared realtime infrastructure for the API app."""
    await realtime.start(ws_manager)
    try:
        yield
    finally:
        await realtime.stop()
...
app = FastAPI(title='Ogham Chat API', version='0.1.0', lifespan=lifespan)
...
realtime = RedisRealtimeCoordinator(redis_url)

v1.include_router(health_router)
v1.include_router(create_messages_router(ws_manager, store))
v1.include_router(create_ws_router(ws_manager, store, realtime))
